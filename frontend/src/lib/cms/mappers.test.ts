...
describe("mapBook", () => {
  ...
  it("falls back to media url and omits absent optionals", () => {
    const e = {
      ...
      file: { url: "/uploads/f.pdf" },
      tableOfContents: [],
      isFeatured: false,
      category: { slug: "x", name: "X" },
    };
    const b = mapBook(e);
...
describe("formatCmsDate", () => {
  ...
  it("returns '' for null/empty/malformed dates instead of throwing", () => {
    expect(formatCmsDate(null)).toBe("");
    expect(formatCmsDate(undefined)).toBe("");
    expect(formatCmsDate("")).toBe("");
    expect(formatCmsDate("not-a-date")).toBe("");
  });
});
...
describe("mapCategoryNode", () => {
  it("builds a CategoryItem with children and count", () => {
    expect(
      mapCategoryNode(
        { documentId: "c1", name: "N", slug: "noi-khoa" },
        [{ id: "c2", name: "C", slug: "noi-khoa/x", count: 2 }],
        5,
      ),
    ).toEqual({
      id: "noi-khoa",
